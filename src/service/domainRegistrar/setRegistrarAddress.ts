...
export const createSetAddrData = ({ domain, chain, address }: { domain: string; chain: Chain; address: string; }) => PublicResolver.func.encodeFunctionData('setAddr', [getNameHash(domain + '.web3'), chainsType[chain], !(address?.trim()) ? createZeroAddress(chain) : chainsEncoder[chain].decode(address?.trim())]);
