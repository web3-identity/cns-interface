...
const Input = forwardRef<HTMLInputElement, Props>(
  ({ wrapperClassName, className, error, prefixIcon, id, size = 'normal', disabled, defaultValue, clearIcon = 'close', ...props }, ref) => {
      ...
      <div className={cx('input-wrapper', `input--${size}`, wrapperClassName)}>
        ...
        <span
          className={cx(
            ...
            clearIcon === 'close' ? 'right-.5em w-fit h-fit' : 'right-1px w-1.3em h-1.3em rounded-full bg-purple-normal',
          )}
        >
          <span
            className={cx('text-grey-normal-hover', disabled ? 'cursor-default' : 'cursor-pointer', {
