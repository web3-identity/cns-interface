import { isServer, detectOS, getEventListenerOptions } from './util';
...
const setOverflowHiddenPc = () => {
  ...
  return () => {
    ['overflow', 'boxSizing', 'paddingRight'].forEach((x: any) => {
...
const setOverflowHiddenMobile = () => {
  ...
  return () => {
    ...
    ['top', 'width', 'height', 'overflow', 'position'].forEach((x: any) => {
      $body.style[x] = bodyStyle[x] || '';
    });
